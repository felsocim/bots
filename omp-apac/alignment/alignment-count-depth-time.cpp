...
#pragma omp task default(shared) depend(in : A, B, M, N, displ, g, gh, last_print, matrix, matrix[0], matrix[0][0], midi, midj, print_ptr, seq1, seq2, tb, te) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if ((__apac_count_ok || __apac_depth_ok) && 1.74049635508e-05 + seq2 * seq1 * 1.02237940434e-05 > __apac_cutoff)
        {
...
#pragma omp task default(shared) depend(in : A, B, M, N, displ, g, gh, last_print, matrix, matrix[0], matrix[0][0], midi, midj, print_ptr, seq1, seq2, tb, te) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if ((__apac_count_ok || __apac_depth_ok) && 0.0015114047609 + seq2 * (A + midi + 1) * 7.9459780275e-06 > __apac_cutoff)
        {
          if (__apac_count_ok || __apac_depth_ok) {
            __apac_depth = __apac_depth_local + 1;
          }
...
#pragma omp task default(shared) depend(in : displ, g[0], gh[0], matrix, matrix[0], matrix[0][0], sb1[0], sb2[0], se1[0], se2[0], seq1[0], seq2[0]) depend(inout : displ[0], last_print[0], mm_score, print_ptr[0]) firstprivate(__apac_depth_local) if ((__apac_count_ok || __apac_depth_ok) && -0.00107178819208 + *sb2 * *sb1 * 5.28417565398e-06 > __apac_cutoff) firstprivate(se1) firstprivate(se2) firstprivate(sb1) firstprivate(sb2) firstprivate(seq1) firstprivate(seq2) firstprivate(g) firstprivate(gh) firstprivate(print_ptr) firstprivate(last_print)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : se1[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(se1, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : se2[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(se2, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : sb1[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(sb1, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : sb2[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(sb2, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : maxscore[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(maxscore, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : seq1[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(seq1, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : seq2[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(seq2, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : g[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(g, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : gh[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(gh, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : displ[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(displ, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : print_ptr[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(print_ptr, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : last_print[0]) if (__apac_count_ok || __apac_depth_ok) firstprivate(last_print, __apac_depth_local)
            {
              if (__apac_count_ok || __apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
