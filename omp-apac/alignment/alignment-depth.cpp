...
#pragma omp taskgroup
    {
      ...
      if (N <= 0) {
        if (M > 0) {
#pragma omp task default(shared) depend(in : M, displ, last_print, print_ptr) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
          {
            if (__apac_depth_ok) {
...
#pragma omp taskwait
      ...
      if (M <= 1) {
        if (M <= 0) {
#pragma omp task default(shared) depend(in : N, displ, last_print, print_ptr) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
          {
            if (__apac_depth_ok) {
              __apac_depth = __apac_depth_local + 1;
            }
...
#pragma omp taskwait
        ...
        midj = 0;
        for (j = 1; j <= N; j++) {
#pragma omp task default(shared) depend(in : A, B, N, gh, seq1, seq2, tb, te) depend(inout : hh) firstprivate(__apac_depth_local, j) if (__apac_depth_ok)
          {
            if (__apac_depth_ok) {
              __apac_depth = __apac_depth_local + 1;
            }
...
#pragma omp task default(shared) depend(in : N, displ, last_print, print_ptr) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
          ...
          if (midj > 1) {
#pragma omp task default(shared) depend(in : displ, last_print, midj, print_ptr) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp taskwait depend(in : displ, last_print, print_ptr) depend(inout : displ[(*print_ptr)++], last_print[0], print_ptr[0])
          ...
          if (midj < N) {
#pragma omp task default(shared) depend(in : N, displ, last_print, midj, print_ptr) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(in : A, B, s, seq1, seq2) depend(inout : hh) firstprivate(__apac_depth_local, j, i) if (__apac_depth_ok)
          {
            if (__apac_depth_ok) {
              __apac_depth = __apac_depth_local + 1;
            }
...
#pragma omp taskwait depend(in : j) depend(inout : midj)
      ...
      if (type == 1) {
#pragma omp task default(shared) depend(in : A, B, M, N, displ, g, gh, last_print, midi, midj, print_ptr, seq1, seq2, tb, te) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
        {
          if (__apac_depth_ok) {
            __apac_depth = __apac_depth_local + 1;
          }
...
#pragma omp task default(shared) depend(in : A, B, M, N, displ, g, gh, last_print, midi, midj, print_ptr, seq1, seq2, tb, te) depend(inout : displ[0], last_print[0], print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
    ...
    return diff_seq(A, B, M, N, tb, te, print_ptr, last_print, displ, seq1, seq2, g, gh);
  }
}
...
#pragma omp task default(shared) depend(in : def_aa_xref) depend(inout : maxres) firstprivate(__apac_depth_local) if (__apac_depth_ok)
      {
        if (__apac_depth_ok) {
          __apac_depth = __apac_depth_local + 1;
        }
...
#pragma omp taskwait depend(in : sj, seq2) depend(inout : seq2[0])
            *seq2 = sj + 1;
#pragma omp task default(shared) depend(in : g[0], gh[0], m, n, seq1[0], seq2[0], seq_array, seq_array[*seq1], seq_array[*seq1][0], seq_array[*seq2], seq_array[*seq2][0], se1, se2, maxscore, seq1, seq2, g, gh) depend(inout : maxscore[0], se1[0], se2[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(in : g[0], gh[0], maxscore[0], se1[0], se2[0], seq1[0], seq2[0], seq_array, seq_array[*seq1], seq_array[*seq1][0], seq_array[*seq2], seq_array[*seq2][0], se1, se2, sb1, sb2, maxscore, seq1, seq2, g, gh) depend(inout : sb1[0], sb2[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp taskwait depend(inout : last_print[0])
            *last_print = 0;
#pragma omp task default(shared) depend(in : displ, g[0], gh[0], sb1[0], sb2[0], se1[0], se2[0], seq1[0], seq2[0], se1, se2, sb1, sb2, seq1, seq2, g, gh, print_ptr, last_print) depend(inout : displ[0], last_print[0], mm_score, print_ptr[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : se1) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : se2) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : sb1) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : sb2) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : maxscore) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : seq1) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : seq2) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : g) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : gh) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : displ) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : print_ptr) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp task default(shared) depend(inout : last_print) firstprivate(__apac_depth_local) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
...
#pragma omp taskgroup
    {
#pragma omp task default(shared) firstprivate(__apac_depth_local) if (__apac_depth_ok)
      {
        if (__apac_depth_ok) {
          __apac_depth = __apac_depth_local + 1;
        }
