...
#pragma omp taskwait depend(in : HH[j], HH, j) depend(inout : s)
        s = HH[j];
...
#pragma omp taskwait depend(in : RR[j], RR, j) depend(inout : s)
        s = RR[j];
...
#pragma omp taskwait depend(in : N) depend(inout : j)
    for (j = 0; j <= N; j++) {
#pragma omp taskwait depend(in : HH[j], RR[j], HH, RR, j) depend(inout : hh)
      hh = HH[j] + RR[j];
#pragma omp taskwait depend(in : hh) depend(inout : midh)
      if (hh >= midh) {
#pragma omp taskwait depend(in                                                \
...
#pragma omp taskwait depend(inout : j)
    for (j = N; j >= 0; j--) {
#pragma omp taskwait depend(in : DD[j], SS[j], DD, SS, g, j) depend(inout : hh)
      hh = DD[j] + SS[j] + g;
#pragma omp taskwait depend(in : hh) depend(inout : midh)
      if (hh > midh) {
#pragma omp taskwait depend(in : hh) depend(inout : midh)
...
#pragma omp taskwait depend(inout : last_print, print_ptr)
          int *print_ptr = new int(), *last_print = new int();
          len2 = 0;
          for (i = 1; i <= m; i++) {
