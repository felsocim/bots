...
#pragma omp task default(shared) depend(in : W, factors, factors[0], in, in[0], nW, out, out[0]) depend(inout : W[0]) firstprivate(__apac_depth_local, m, k) if ((__apac_count_ok || __apac_depth_ok) && -0.000858793339889 + nW * 3.17687038829e-07 > __apac_cutoff)
        {
          if (__apac_count_ok || __apac_depth_ok) {
            __apac_depth = __apac_depth_local + 1;
          }
