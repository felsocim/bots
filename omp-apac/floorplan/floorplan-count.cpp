...
#pragma omp task default(shared) depend(in : CELLS, NWS, id) depend(inout : CELLS[0], NWS[0], NWS[0][0], nn) firstprivate(i) if (__apac_count_ok)
      {
...
#pragma omp task default(shared) depend(in : board, cells, footprint, footprint[0], id) depend(inout : board[0], board[0][0], cells[0], nnc) if (__apac_count_ok)
            {
...
#pragma omp task default(shared) depend(inout : cells) if (__apac_count_ok)
        {
          delete[] cells;
          if (__apac_count_ok) {
#pragma omp atomic
...
#pragma omp task default(shared) depend(in : board, footprint, footprint[0], gcells) depend(inout : board[0], board[0][0], gcells[0]) if (__apac_count_ok)
    {
#pragma omp critical
