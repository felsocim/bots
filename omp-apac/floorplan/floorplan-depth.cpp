...
#pragma omp taskgroup
    {
      ...
      nnc = nnl = 0;
      for (i = 0; i < CELLS[id].n; i++) {
#pragma omp task default(shared) depend(in : CELLS, NWS, id) depend(inout : CELLS[0], NWS[0], NWS[0][0], nn) firstprivate(__apac_depth_local, i) if (__apac_depth_ok)
        {
          if (__apac_depth_ok) {
...
#pragma omp critical
            if (area < MIN_AREA) {
#pragma omp task default(shared) depend(in : board, cells, footprint, footprint[0], id) depend(inout : board[0], board[0][0], cells[0], nnc) firstprivate(__apac_depth_local) if (__apac_depth_ok)
              {
                if (__apac_depth_ok) {
                  __apac_depth = __apac_depth_local + 1;
                }
        ...
        _end:;
#pragma omp task default(shared) depend(inout : cells) firstprivate(__apac_depth_local) if (__apac_depth_ok)
          {
            if (__apac_depth_ok) {
              __apac_depth = __apac_depth_local + 1;
            }
...
#pragma omp taskgroup
    {
      ...
      footprint[1] = 0;
      bots_message("Computing floorplan ");
#pragma omp task default(shared) depend(in : board, footprint, footprint[0], gcells) depend(inout : board[0], board[0][0], gcells[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
      {
        if (__apac_depth_ok) {
          __apac_depth = __apac_depth_local + 1;
        }
