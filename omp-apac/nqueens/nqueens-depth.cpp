...
#pragma omp taskwait depend(in : b, j) depend(inout : b[0])
        if (ok(j + 1, b)) {
#pragma omp task default(shared) depend(in : b, csols, j, n) depend(inout : b[0], csols[i]) firstprivate(__apac_depth_local, i) if (__apac_depth_ok)
          {
            if (__apac_depth_ok) {
...
#pragma omp critical
      ...
      char* a;
      a = (char*)__builtin_alloca(size * sizeof(char));
#pragma omp task default(shared) depend(in : a, size) depend(inout : a[0], total_count) firstprivate(__apac_depth_local) if (__apac_depth_ok)
      {
        if (__apac_depth_ok) {
          __apac_depth = __apac_depth_local + 1;
        }
