...
#pragma omp taskgroup
    {
      ...
      if (right_limit <= 256) {
        ...
        int* pivot = new int();
        partition(pivot, in_out_data, right_limit);
#pragma omp task default(shared) depend(in : in_out_data, pivot[0]) depend(inout : in_out_data[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok && -0.000671212615169 + *pivot * 2.40240038622e-07 > __apac_cutoff) firstprivate(pivot)
        {
          if (__apac_depth_ok) {
...
#pragma omp task default(shared) depend(in : in_out_data, pivot[0], right_limit) depend(inout : in_out_data[*pivot + 1]) firstprivate(__apac_depth_local) if (__apac_depth_ok && -0.000854619699579 + (right_limit - (*pivot + 1)) * 2.44868432938e-07 > __apac_cutoff) firstprivate(pivot)
        {
          if (__apac_depth_ok) {
            __apac_depth = __apac_depth_local + 1;
          }
