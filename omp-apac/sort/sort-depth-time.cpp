...
#pragma omp taskgroup
    {
      ...
      tmpC = tmpB + quarter;
      tmpD = tmpC + quarter;
#pragma omp task default(shared) depend(in : low[0], quarter, size) depend(inout : low, tmp) firstprivate(__apac_depth_local) if (__apac_depth_ok && 0.00244808459551 + (size - 3 * quarter) * 3.26227615127e-06 > __apac_cutoff)
      {
        if (__apac_depth_ok) {
