...
#pragma omp task default(shared) depend(in : high, low) depend(inout : high[0], low[0], p, p[0])
      {
        p = seqpart(low, high);
        seqquick(low, p);
        low = p + 1;
...
#pragma omp task default(shared) depend(in : high, high[0]) depend(inout : p)
...
#pragma omp task default(shared) depend(in : lowdest) depend(inout : high1, high2, low1, low2, lowdest[0])
...
#pragma omp task default(shared) depend(in : high2, lowdest) depend(inout : high1, low1, low2, lowdest[0], lowdest[lowsize + 1], split2, split2[0])
    {
...
#pragma omp task default(shared) depend(in : low, size) depend(inout : low[0])
...
#pragma omp taskwait depend(inout : low)
    ...
    tmpC = tmpB + quarter;
    tmpD = tmpC + quarter;
#pragma omp task default(shared) depend(in : low[0], quarter, size) depend(inout : low, tmp)
    {
