...
#pragma omp taskgroup
    {
      ...
      for (int kk = 0; kk < bots_arg_size; kk++) {
#pragma omp task default(shared) depend(in : BENCH, BENCH[kk * bots_arg_size + kk]) depend(inout : BENCH[kk * bots_arg_size + kk][0]) firstprivate(__apac_depth_local, kk) if (__apac_depth_ok)
        {
          if (__apac_depth_ok) {
...
#pragma omp task default(shared) depend(in : BENCH, BENCH[kk * bots_arg_size + jj], BENCH[kk * bots_arg_size + kk], BENCH[kk * bots_arg_size + kk][0]) depend(inout : BENCH[kk * bots_arg_size + jj][0]) firstprivate(__apac_depth_local, kk, jj) if (__apac_depth_ok)
            {
              if (__apac_depth_ok) {
                __apac_depth = __apac_depth_local + 1;
              }
