...
void bdiv(const float* diag, float* row) {
  int i, j, k;
  for (i = 0; i < bots_arg_size_1; i++)
    for (k = 0; k < bots_arg_size_1; k++) {
...
void fwd(const float* diag, float* col) {
  int i, j, k;
  for (j = 0; j < bots_arg_size_1; j++)
    for (k = 0; k < bots_arg_size_1; k++)
      for (i = k + 1; i < bots_arg_size_1; i++)
...
#pragma omp task default(shared)                    \
    depend(in                                       \
           : BENCH, BENCH[kk * bots_arg_size + kk]) \
        depend(inout                                \
               : BENCH[kk * bots_arg_size + kk][0]) firstprivate(kk)
...
#pragma omp task default(shared) depend(                                     \
    in                                                                       \
    : BENCH, BENCH[kk * bots_arg_size + jj], BENCH[kk * bots_arg_size + kk], \
      BENCH[kk * bots_arg_size + kk][0])                                     \
    depend(inout                                                             \
           : BENCH[kk * bots_arg_size + jj][0]) firstprivate(kk, jj)
...
#pragma omp task default(shared) depend(                                     \
    in                                                                       \
    : BENCH, BENCH[ii * bots_arg_size + kk], BENCH[kk * bots_arg_size + kk], \
      BENCH[kk * bots_arg_size + kk][0])                                     \
    depend(inout                                                             \
           : BENCH[ii * bots_arg_size + kk][0]) firstprivate(kk, ii)
...
#pragma omp task default(shared) depend(in       \
                                        : BENCH) \
    depend(inout                                 \
           : BENCH[ii * bots_arg_size + jj]) firstprivate(jj, ii)
                BENCH[ii * bots_arg_size + jj] = allocate_clean_block();
              }
#pragma omp task default(shared) depend(                                     \
    in                                                                       \
    : BENCH, BENCH[ii * bots_arg_size + jj], BENCH[ii * bots_arg_size + kk], \
      BENCH[ii * bots_arg_size + kk][0], BENCH[kk * bots_arg_size + jj],     \
      BENCH[kk * bots_arg_size + jj][0])                                     \
    depend(inout                                                             \
           : BENCH[ii * bots_arg_size + jj][0]) firstprivate(kk, jj, ii)
