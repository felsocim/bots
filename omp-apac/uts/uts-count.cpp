...
#pragma omp task default(shared) depend(in : depth, partialCount) depend(inout : n, partialCount[i]) firstprivate(i) if (__apac_count_ok)
      {
...
#pragma omp task default(shared) depend(inout : root, root[0]) if (__apac_count_ok)
    {
...
#pragma omp task default(shared) depend(in : root) depend(inout : num_nodes, root[0]) if (__apac_count_ok)
    {
