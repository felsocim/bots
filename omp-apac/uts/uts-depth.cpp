...
#pragma omp task default(shared) depend(in : depth, partialCount) depend(inout : n, partialCount[i]) firstprivate(__apac_depth_local, i) if (__apac_depth_ok)
        {
          if (__apac_depth_ok) {
...
#pragma omp taskgroup
    {
      long long unsigned int num_nodes = 0;
#pragma omp task default(shared) depend(inout : root, root[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
      {
        if (__apac_depth_ok) {
          __apac_depth = __apac_depth_local + 1;
        }
      ...
      bots_message("Computing Unbalance Tree Search algorithm ");
#pragma omp task default(shared) depend(in : root) depend(inout : num_nodes, root[0]) firstprivate(__apac_depth_local) if (__apac_depth_ok)
      {
        if (__apac_depth_ok) {
          __apac_depth = __apac_depth_local + 1;
        }
