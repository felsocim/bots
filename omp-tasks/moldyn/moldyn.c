...
#pragma omp task firstprivate(me, idx_z, idx_y, idx_x)
          {
...
void fill_cell_with_rand_particles(Cell* inCell, double box_width, int size) {
  ...
  for (int idx = 0; idx < size; idx++) {
    ...
    particle.x = box_width * genrand_real2();
    particle.y = box_width * genrand_real2();
    particle.z = box_width * genrand_real2();
    particle.weight = 1.;
    particle.vx = 0.;
    particle.vy = 0.;
    particle.vz = 0.;
    Particle_forces particle_forces;
    particle_forces.fx = box_width * genrand_real2();
    particle_forces.fy = box_width * genrand_real2();
    particle_forces.fz = box_width * genrand_real2();
